...
std::string MapAnalysisManager::getFileNameForChokepoint() 
{
	...
	return Broodwar->mapHash() + separator + x + separator + y + ".txt";
}
...
void MapAnalysisManager::init()
{
	...
	if (stat(Options::FileIO::FILE_SETTINGS, &buf) != -1)
	{
		...
		std::string solutionFile = readDir + getFileNameForChokepoint();
		
		//if outputFile doesn't exist, this is the first time on this chokepoint
		if (stat(solutionFile.c_str(), &buf) == -1)
		{
			analyzeChoke();
			initClingoProgramSource();
			solutionFile = runASPSolver();
		}
...
{
	std::string writeFile = tempDir + "ITUBotWall.txt";
...
std::string MapAnalysisManager::runASPSolver()
{
	// requires Clingo 3.0.5

	std::string clingo = "bwapi-data\\AI\\clingo.exe ";
	std::string problemPart = tempDir + "ITUBotWall.txt > ";
	std::string outputFileName = getFileNameForChokepoint();
	std::string outputFile = tempDir + outputFileName;

	std::string combined = clingo + problemPart + outputFile;

	system(combined.c_str());

	boost::filesystem::path tempOutputPath(outputFile);
	boost::filesystem::path writeOutputFile(writeDir + outputFileName);

	copy_file(tempOutputPath, writeOutputFile);

	return outputFile;
}

void MapAnalysisManager::readSolution(std::string outputFile)
{
