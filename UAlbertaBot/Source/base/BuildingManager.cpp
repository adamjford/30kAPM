...
typedef std::pair<BWAPI::UnitType, BWAPI::TilePosition> WallInLocation;
...
BWAPI::TilePosition BuildingManager::getBuildingLocation(const Building & b)
{
    ...
    // special case for wall-in strategies
    else if (StrategyManager::Instance().getCurrentStrategy() == StrategyManager::TerranBunkerBuild
        && (b.type == BWAPI::UnitTypes::Terran_Supply_Depot || b.type == BWAPI::UnitTypes::Terran_Barracks))
    {
        // check to see if a wall-in building needs to be built
        std::vector<WallInLocation> wall = MapAnalysisManager::Instance().getWallInPositions();

    	BOOST_FOREACH(WallInLocation & pair, wall)
        {
            if (pair.first == b.type && BuildingPlacer::Instance().canBuildHereWithSpace(pair.second, b, 0))
            {
                return pair.second;
            }
        }
